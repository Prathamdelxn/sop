...
const SearchBar = ({ searchQuery, onSearchChange, onFilter, onTypeFilterChange, onStatusFilterChange, typeFilter, statusFilter }) => {
  ...
  <div className="relative shrink-0">
    <button
      ...
      className="flex items-center gap-2 px-6 py-4 bg-white/80 backdrop-blur-sm border-0 rounded-2xl hover:bg-white transition-all duration-300 shadow-sm hover:shadow-xl text-slate-700 hover:scale-105 whitespace-nowrap"
    >
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Calculate summary stats
        const pending = pendingTasks.filter(t => t.status.toLowerCase() === 'pending').length;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
  <StatCard icon={AlertCircle} label="Pending W" value={workSummary.pendingReviews} color="orange" />
  <StatCard icon={CheckCircle} label="Approved Reviews" value={workSummary.approvedThisWeek} color="green" />
  <StatCard icon={XCircle} label="Rejected Reviews" value={workSummary.rejectedThisWeek} color="red" />
  <StatCard icon={AlertCircle} label="Deviation Raised" value={workSummary.deviationRaised} color="yellow" />
</div>
 
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="bg-gray-50 text-gray-900 px-6 py-6 flex justify-between items-center border-b border-gray-200">
            <div>
              <h2 className="text-xl font-bold mb-1">Equipment Tasks</h2>
              <p className="text-gray-500">Monitor and manage equipment lifecycle</p>
            </div>
          </div>
