    ...
    if (isEditMode && id) {
      const fetchChecklist = async () => {
        try {
          ...
          // Merge defaultStage + stages for edit UI (default first)
const rawStages = data?.defaultStage
  ? [data.defaultStage, ...(data.stages || [])]
  : (data.stages || []);
const stagesWithIds = addIdsToStructure(rawStages);
setStages(stagesWithIds);
          // Select the first stage if exists
          if (stagesWithIds.length > 0) {
          setSelectedStageId(stagesWithIds[0].id);
  ...
  const renderTableRows = () => {
    return tableData.map((item) => (
      <tr key={item.id} className="border-b border-gray-200 hover:bg-gray-50">
        ...
        <td className="py-3 px-4 text-sm text-gray-700">
          <select
            value={item.cleaningStatus}
            onChange={(e) => handleCleaningStatusChange(item.id, e.target.value)}
            className="w-full px-2 py-1 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 transition-colors focus:border-blue-500 bg-white"
          >
  ...
  const handleSubmit = async () => {
    ...
    // New validation for Visual Representation (only if enabled)
...
const stagesWithoutDefault = schemaStages.length > 0
 ...
 const handleStageCountConfirm = () => {
   ...
   const maxStageNumber = stages.reduce((max, stage) => {
     if (stage.title.startsWith("Stage ")) {
       const num = parseInt(stage.title.split(" ")[1]);
   ...
   const newStages = Array.from({ length: stageCount }, (_, idx) => ({
     id: generateId("stage"), // Use generateId for unique IDs
     title: `Stage ${maxStageNumber + idx + 1}`, // Continue sequential numbering
     tasks: [],
   }));
   setStages((prev) => [...prev, ...newStages]); // Append to existing stages
   setShowStageCountModal(false);
    ...
    <div className="min-h-screen bg-slate-100 p-4 sm:p-6 lg:p-8">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="lg:col-span-1 bg-white rounded-xl shadow-md p-6">
  {/* Stages header (for regular stages) */}
  <div className="flex items-center justify-between mb-6">
    <h2 className="text-lg font-semibold text-slate-900">Stages</h2>
    <span className="text-xs text-slate-500 bg-slate-100 px-2 py-1 rounded-full">
      {stages.length > 1 ? stages.length - 1 : 0}
    </span>
  </div>
  
  {/* New Stage button for regular stages */}
  <button
    onClick={() => setShowStageCountModal(true)}
    className="w-full px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center gap-2 mb-4"
  >
  ...
  {/* Regular stages (excluding default stage) */}
  <DndContext
    sensors={sensors}
    collisionDetection={closestCenter}
    onDragStart={handleStageDragStart}
  ...
  >
    <SortableContext
      items={stages.length > 1 ? stages.slice(1).map((s) => s.id) : []}
      strategy={verticalListSortingStrategy}
    >
      <div className="space-y-2">
        {stages.length > 1 && stages.slice(1).map((stage, idx) => (
          <div key={stage.id} className="relative group/stage">
            <SortableItem
              id={stage.id}
              title={stage.title}
              ...
              onAddSubtask={() => {}}
              onDelete={handleDeleteStage}
              numbering={idx + 1} // Start from 1 for new stages
              showActionButtons={false}
              onClick={(id) => {
                setSelectedStageId(id);
                setShowVisualTable(false);
        ...
        {stages.length <= 1 && (
          <div className="text-center py-4 text-slate-500 text-sm">
            No stages yet. Click "New Stage" to add stages.
          </div>
        )}
      </div>
    </SortableContext>
    <DragOverlay className="z-50">
      {activeStageItem && activeStageItem.id !== stages[0]?.id ? (
        <div className="p-3 bg-white rounded-lg shadow-xl border border-slate-200">
          <div className="font-medium text-slate-900 text-sm">
            {activeStageItem.title}
          </div>
  ...
  {/* Default Stage section - appears below Visual Representation */}
  <div className="mt-4 pt-4 border-t border-slate-200">
    <div className="flex items-center justify-between mb-4">
      {/* <h3 className="text-md font-semibold text-slate-900">Default Stage</h3> */}
      {/* No number badge for default stage */}
    </div>
    ...
    {stages.length > 0 && (
      <div className="relative">
        <div
          ...
          className="group p-3 rounded-lg border border-slate-200 bg-white hover:border-slate-300 hover:shadow-sm transition-all duration-200 cursor-pointer"
        >
          <div className="flex items-start gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    {/* NO NUMBERING FOR DEFAULT STAGE */}
                    <h3 className="text-sm font-medium text-slate-900 leading-tight truncate w-[150px]">
                      {stages[0].title}
                    </h3>
                  </div>
                  <p className="text-xs text-slate-600 mb-1 truncate">
                    {stages[0].tasks?.length || 0} tasks
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            onClick={(e) => {
              e.stopPropagation();
              handleDuplicateStage(stages[0].id);
            }}
            className="p-1.5 text-slate-500 hover:text-green-600 hover:bg-green-50 rounded-md transition-colors"
            title={`Duplicate "${stages[0].title}"`}
          >
            <Copy className="w-3 h-3" />
          </button>
        </div>
