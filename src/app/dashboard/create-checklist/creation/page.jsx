            ...
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h3 className={`text-sm font-medium leading-tight truncate w-[150px] ${isStopChecked ? 'text-red-600' : 'text-slate-900'
                    }`}>
                    {title}
                  </h3>
                </div>
                {description && (
                  <p className={`text-xs mb-1 truncate ${isStopChecked ? 'text-red-500' : 'text-slate-600'
                    }`}>
                    {description}
                  </p>
                )}
      ...
      return (
        <tr key={item.id} className="border-b border-gray-200 hover:bg-gray-50">
          ...
          <td className="py-3 px-4 text-sm text-gray-700">
            ...
            >
              <option value="Visually Clean">Clean</option>
    ...
    return (
      <>
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={() => toggleTaskForm(stageId)}
            className="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 flex items-center gap-2"
          >
            <Plus className="w-4 h-4" /> Add Task
          </button>
        </div>
        {showTaskForms[stageId] && (
            ...
            {bulkTasks.map((task, index) => (
              ...
              >
                ...
                <InputField
                  ...
                  value={task.title || ""}
                  onChange={(e) => handleTaskInputChange(stageId, index, e)}
                  required
                  error={errors.taskForms[`${stageId}_${index}`]?.title}
                  items={stage.tasks || []}
                  itemType="Task"
                />
                <TextAreaField
                ...
                {showTimeFields[`${stageId}_${index}`] && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3 bg-white rounded-lg border border-slate-200">
                    ...
                    <ErrorMessage
                      message={errors.taskForms[`${stageId}_${index}`]?.time}
                    />
                  </div>
                )}
                <div className="flex gap-2 flex-wrap">
                  <button
                    onClick={() => addTask(stageId, index)}
                    className="px-3 py-1.5 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition-colors"
                  >
                    Add Task
                  </button>
                  ...
                  <button
                    onClick={() => handleOpenTaskImageModal(stageId, index)}
                    className="px-3 py-1.5 bg-green-100 text-green-700 hover:bg-green-200 text-sm rounded-lg flex items-center gap-1"
                  >
                    <ImageIcon className="w-4 h-4" />
            ...
            {bulkTasks.length > 0 && (
              <button
                onClick={() => {
                  ...
                  setBulkTasks([]);
                  bulkTasks.forEach((_, index) => clearTaskErrors(stageId, index));
                }}
                className="px-3 py-1.5 bg-red-100 text-red-700 hover:bg-red-200 text-sm rounded-lg"
              >
                Cancel All Task
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 bg-white rounded-xl shadow-md p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-xs text-slate-500 bg-slate-100 px-2 py-1 rounded-full">
                {stages.filter((s) => s.title !== "Default Stage").length}
              </span>
            </div>
            <button
            ...
            >
              <SortableContext
                items={stages.filter((s) => s.title !== "Default Stage").map((s) => s.id)}
                strategy={verticalListSortingStrategy}
              >
            ...
            {stages.find((s) => s.title === "Default Stage") && (
              <button
                onClick={() => {
                  const defaultStage = stages.find((s) => s.title === "Default Stage");
                  if (defaultStage) {
              ...
              >
                ...
                <div className="text-xs text-slate-500 mt-1">
                  {stages.find((s) => s.title === "Default Stage")?.tasks?.length || 0} tasks
                </div>
              </button>
            )}
          </div>
          <div className="lg:col-span-2 bg-white rounded-xl shadow-md p-6">
            {showVisualTable ? (
              ...
              renderStageContent(selectedStageId)
            ) : null}
            {showVisualTable && (
              <div className="mt-8 border-t border-gray-200 pt-8">
                {/* Visual table content is sufficient, removing duplicate render */}
              </div>
            )}
            {!showVisualTable && !selectedStageId && (
