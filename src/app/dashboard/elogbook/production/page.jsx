...
export default function ProductionPage() {
  ...
  // Fetch Active Batch
  useEffect(() => {
    if (selectedPart && selectedLineId && isClient) {
      fetchActiveBatch(selectedPart);
    } else {
      setActiveBatch(null);
    }
  ...
  const onStartBatch = async () => {
    if (!selectedMasterData) return alert('Please select a part first');
    await handleStartBatch({
      masterDataId: selectedMasterData._id,
      startUser: userData?.name || userData?.username,
    });
    ...
    <div className="min-h-screen p-4 sm:p-6 lg:p-8 bg-gray-50">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-extrabold text-gray-900">Production Cycle</h1>
              ...
              <select value={selectedCustomer} onChange={(e) => handleCustomerChange(e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                {customers.map((c) => (
                  <option key={c.customerName} value={c.customerName}>
                    {c.customerName} {c.subCompany ? `(${c.subCompany})` : ''}
                  </option>
                ))}
              </select>
            </div>
      ...
      {selectedMasterData && (
        <div className="flex flex-wrap gap-3 mb-6">
          {selectedPlant && <div className="bg-emerald-100 text-emerald-700 px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2"><Factory className="w-4 h-4" />{selectedPlant.name}</div>}
          {selectedLine && <div className="bg-blue-100 text-blue-700 px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2"><GitBranch className="w-4 h-4" />Line {selectedLine.lineNumber}</div>}
          <div className="bg-purple-100 text-purple-700 px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2"><Package className="w-4 h-4" />{selectedMasterData.partName}</div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-16 text-center border border-gray-100">
          ...
          <h3 className="text-xl font-bold text-gray-700">No Baskets Found</h3>
          <p className="text-gray-500 mt-2">Select all fields and start a production batch to begin</p>
        </div>
      ) : (
