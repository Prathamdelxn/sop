...
export default function ProductionPage() {
  ...
  const { lines, refetch: fetchLines } = useLines(
    userData?.companyId,
  ...
  }, [userData?.companyId, fetchPlants]);
  ...
  useEffect(() => {
    ...
    if (savedPlant) setSelectedPlantId(savedPlant);
  ...
  // Auto-fetch Active Batch when Part & Line are selected
  useEffect(() => {
    if (selectedPart && selectedLineId) {
      fetchActiveBatch(selectedPart);
    } else {
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 lg:p-8">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-6 shadow-sm">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      {activeBatch ? `Started: ${new Date(activeBatch.startTime).toLocaleString()}` : 'Start a batch to begin production sequence'}
                    </p>
                  </div>
                </div>
                <div>
                  {!activeBatch ? (
                    <button
                      onClick={onStartBatch}
                      disabled={batchActionLoading === 'batch'}
                      className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-indigo-500 to-blue-600 text-white rounded-xl font-bold text-sm shadow-lg hover:shadow-xl disabled:opacity-50"
                    >
                      {batchActionLoading === 'batch' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
                      Start Production Batch
                    </button>
                  ) : (
                    <button
                      onClick={onEndBatch}
                      disabled={batchActionLoading === 'batch'}
                      className="flex items-center gap-2 px-6 py-2.5 bg-white text-red-600 border-2 border-red-100 rounded-xl font-bold text-sm hover:bg-red-50 disabled:opacity-50"
                    >
                      {batchActionLoading === 'batch' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Square className="w-4 h-4" />}
                      End Production Batch
                    </button>
                  )}
                </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-end gap-4">
            <div className="flex-1">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={isExecutionInterlocked ? `Finish Basket ${activeBasket.basketNumber} first` : "Scan barcode or type basket number + Enter"}
                  disabled={isExecutionInterlocked}
                  className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400 disabled:bg-gray-50"
                />
              </div>
            </div>
            ...
            <button
              onClick={() => {
                ...
                if (isExecutionInterlocked) return alert(`Cannot start new basket. Please end Basket ${activeBasket.basketNumber} first.`);
                setStartBasketNumber(String(baskets.length + 1));
                setShowStartModal(true);
              }}
              disabled={!activeBatch || isExecutionInterlocked}
              className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl font-semibold text-sm shadow-lg hover:shadow-xl disabled:opacity-50 whitespace-nowrap"
            >
        ...
        {selectedMasterData && (
          <div className="flex flex-wrap gap-3 mt-4 pt-4 border-t border-gray-100">
            {selectedPlant && <div className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 rounded-lg text-xs font-medium text-emerald-700"><Factory className="w-3.5 h-3.5" /> {selectedPlant.name}</div>}
            {selectedLine && <div className="flex items-center gap-1.5 px-3 py-1.5 bg-cyan-50 rounded-lg text-xs font-medium text-cyan-700"><GitBranch className="w-3.5 h-3.5" /> Line {selectedLine.lineNumber}</div>}
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 rounded-lg text-xs font-medium text-indigo-700"><Building2 className="w-3.5 h-3.5" /> {selectedMasterData.customerName}</div>
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 rounded-lg text-xs font-medium text-indigo-700"><Package className="w-3.5 h-3.5" /> {selectedMasterData.partName}</div>
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 rounded-lg text-xs font-medium text-blue-700"><Timer className="w-3.5 h-3.5" /> Standard: {selectedMasterData.standardCycleTime} min</div>
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 rounded-lg text-xs font-medium text-amber-700"><Zap className="w-3.5 h-3.5" /> {selectedMasterData.standardVoltage}V</div>
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 rounded-lg text-xs font-medium text-red-600"><Thermometer className="w-3.5 h-3.5" /> {selectedMasterData.standardTemperature}°C</div>
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-50 rounded-lg text-xs font-medium text-purple-700"><Package className="w-3.5 h-3.5" /> {selectedMasterData.partsPerBasket} parts/basket</div>
          </div>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
          <Package className="w-12 h-12 text-gray-300 mx-auto mb-4" />
          <h3 className="text-lg font-bold text-gray-700">No Baskets Found</h3>
          <p className="text-sm text-gray-400 mt-2">
            {activeBatch ? 'Start your first basket to begin production.' : 'Select plant, line, customer & part to start.'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {baskets.map((basket) => (
            <BasketCard
              ...
              actionLoading={actionLoading}
              canDoQC={canDoQC}
              onStop={(id) => { setStoppingBasketId(id); setShowStopModal(true); }}
              onRestart={handleRestartBasket}
              onEnd={onEndBasket}
            />
          ))}
        </div>
      ...
      <StopReasonModal
        show={showStopModal}
        onClose={() => { setShowStopModal(false); setStopReason(''); }}
        stopReason={stopReason}
        setStopReason={setStopReason}
        onSubmit={onStopBasket}
      />
