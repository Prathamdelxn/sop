...
const prepareAssignmentData = () => {
  ...
  const result = {
    ...
    generatedId: selectedAssignment.generatedId,
    equipmentName: selectedAssignment.equipment?.name || "N/A",
    checklistName: selectedAssignment.prototypeData?.name || "N/A",
    stages: selectedAssignment.prototypeData?.stages?.map((stage, stageIndex) => {
      const stageKey = `stage-${stageIndex}`;
      const stageWorker = assignedWorkers[stageKey] || null;

      return {
        stageId: stage._id || stageIndex,
        ...
        tasks: stage.tasks?.map((task, taskIndex) => {
          ...
          return {
            ...
            subtasks: task.subtasks?.map((subtask, subtaskIndex) => ({
              subtaskId: subtask._id || subtaskIndex,
              subtaskTitle: subtask.title,
              // subtasks can inherit task worker or remain unassigned
              assignedWorker: taskWorker,
              status: taskWorker ? "Assigned" : "Unassigned",
            })) || [],
          };
...
/>
  ...
  <span className="text-sm bg-blue-100 text-blue-800 px-3 py-1 rounded-full font-medium border border-blue-200">
    ...
    onClick={() => {
      const finalData = prepareAssignmentData();
      console.log("📌 Final Assignment Data:", finalData);
      alert("Assignments prepared successfully! Check console.");
      resetModal();
    }}
  >
    Save Assignments
